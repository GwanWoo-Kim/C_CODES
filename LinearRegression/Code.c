...
//code for Linear Regression(LR) mady by KimGwanWoo date20191017
double drand(double low, double high)//make random # in range low~hugh as real #
{
    double d;
...
void delay(clock_t n)  //add some delay for non overlapped random #
{
  clock_t start = clock();
  while(clock() - start < n);//order is ms
}
...
double k;
for(k=1;k<=9;k++)//make several data and figures
{
int n = 1000 ; //#of data
double xmin= -1*k , xmax= +1*k;
double ymin= -1*k , ymax= +1*k;
double M = drand(-3*k,+3*k); //initial slope of data
double x[n],y[n];
double Ycont=drand(-0.1,+0.1);
double Slope=drand(-0.1,+0.1);
double alpha = 1E-4;//move step in gradient descent(GD)
double p;//assumtin, or Prediction
double a;//area divide factor
double RMSE;
double error;
int i =0,j=0;
for(i=0; i < n; i ++) // I consider triplet for data
{       
	x[i]=drand(xmin,xmax);
	delay(1);
	a=drand(-1*k,+1*k);
...
while(j<100000*n) //repeated time for each variation
{
for (i = 0; i < n; i ++) //LR algorithm with GD
{
    p =  Slope * x[i] + Ycont;
    error = p - y[i];
    Ycont = Ycont - alpha * error;		// this part is 
    Slope = Slope - alpha * error * x[i];	// actual GD
    j++;
}
}
...
RMSE=sqrt(RMSE/n);//final evaluation my code with RootMeanSquareError
